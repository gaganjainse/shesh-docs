...
type: explanation
summary: "Two protocols Shesh adopts alongside MCP so it can live in editors and coordinate agents."
audience: operator
status: current
verified: 2026-08-15
---
...
- **vs MCP:** ACP is the outer layer (human-in-editor driving the agent); MCP is the inner
  layer (agent calling tools). Shesh runs both.
- **Component:** `shesh-acp` (P0).

Minimum ACP surface Shesh implements:
...
- **Why:** lets Shesh's specialist subagents talk directly (coordinator→researcher→critic) and,
  later, lets remote agents participate without the project inventing a protocol.
- **Component:** `shesh-orchestrator` speaks A2A on a local Unix socket (P1).

Shesh uses A2A for **local agent messaging first**; remote/cross-org A2A is off by default and requires
