...
type: explanation
summary: "If anything differs, update profile.conf before running setup."
audience: operator
status: current
verified: 2026-08-15
...
## CachyOS-specific tuning (260628)
...
- **NVMe The maintainer/O scheduler:** CachyOS 260628 defaults NVMe to `kyber`. Enforce via udev:
  ```
...
- **Network:** BBR + `cake` (laptop/Wi-Fi benefit):
  ...
  ```
- **PipeWire latency:** 16" laptop audio is fine at quantum 256 (≈5 ms @48k). Do not set 64 (causes
...
## The `msi-mux-switcher.py` improvements to make
...
## Post-install verification checklist (on the real machine)
```bash
# Display
...
If any of these disagree with the profile, fix the profile and the installer, not the running
