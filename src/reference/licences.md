...
type: reference
summary: "Your repo root is GPL-3.0 (matches upstream end-4/dots-hyprland)."
audience: operator
status: current
verified: 2026-08-15
---
...
| NVIDIA driver/userspace | NVIDIA proprietary |  system pkg, not linked into your code | use distro packages |
...
## Pinned versions (install-time manifest)
### System / OS
| Thing | Target | Notes |
|---|---|---|
...
| Quickshell | latest (end-4 pinned via PKGBUILD) | does not override the end-4 version casually |
...
## Every link you provided — audited
| Link | What it is | Verdict / use |
|---|---|---|
| github.com/gaganjainse | your profile | AI/LLM engineer; SheshAOS, SHESH, Vyākṛti, RAG, eval harness |
...
| github.com/criptogus/HermesOffice | AI office suite (Electron, mac/win) | concept only (shared OpenAI endpoint); does not install |
| github.com/diegosouza.../OmniRoute | 40k multi-provider AI gateway | opt-in cloud fallback; never default |
| github.com/earendil-works/pi | agent harness (85k) | reference for agent loop/hardening; does not add runtime |
...
| github.com/PrimeIntellect-ai/prime-agent | RLM harness | adopt "Continual Harness" prompt-refinement idea |
...
## Your own repos to connect (from your profile)
...
## Upgrade checklist (run monthly on rolling release)
```bash
