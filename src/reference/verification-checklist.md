...
type: reference
summary: "Everything Shesh automates is unit-tested, but some things cannot be verified."
audience: operator
status: current
verified: 2026-08-15
---
...
> Last updated: 2026-08-13 (the full set of sections — added rolling-dependency, security-posture and recovery-drill sections). This file is updated on every autopilot run (now automatic via live_update.py); the companion `docs/history/queries/QUERYLOG.md` records what changed and why.
...
## Voice (shesh-voice / Newelle fork)
...
## GPU, power, and MUX (MSI-specific)
...
## Rolling dependency hygiene (monthly, ~5 minutes)
CachyOS rolls; so do the project. Full protocol: `docs/policies/DEPENDENCY_POLICY.md`.
...
## Security posture (GitHub-side browser checks, quarterly)
...
## Recovery drill (quarterly, ~15 minutes)
...
## Known things that need deliberate (non-autopilot) work
