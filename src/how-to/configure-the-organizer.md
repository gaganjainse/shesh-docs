...
title: 05
type: how-to
summary: "only calls Ollama (phi4-mini) for unknown extensions/content, and moondream2 for images."
audience: operator
status: current
verified: 2026-08-15
---
...
## The Rust watcher (`tools/smart-organizer/watcher-rs/`)
### `Cargo.toml`
```toml
[package]
name = "sm-watcher"
...
## The Python classifier (`tools/smart-organizer/classifier.py`)
```python
#!/usr/bin/env python3
...
## The apply layer (Bash) — the only mover
...
2. If `conf < 0.7` → `notify-send` with **Move / Leave / Always** actions (via `notify-send -A`),
   does not move until the user picks.
...
## Rules file (user-editable, overrides everything)
...
## Systemd units (canonical, no here-docs)
...
This makes "Hey Shesh, organize the downloads folder and undo the last thing" work by voice.

---

## Safety guarantees (write these into the README and tests)
